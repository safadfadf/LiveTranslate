...
from .schema import DecodeResult, ASREngineConfig, TranscribeResult, ForcedAlignItem, ForcedAlignResult
...
class QwenASREngine:
    ...
    def __init__(self, config: ASREngineConfig):
        ...
        logger.info(f"Qwen3-ASR init (DML: {config.use_dml}, Vulkan: {config.vulkan_enable})")
    ...
    def shutdown(self):
        ...
        logger.info("Qwen3-ASR engine shutdown")
    ...
    def _safe_decode(
        ...
        for i in range(4):
            ...
            if not res.is_aborted:
                break
            temperature += 0.3
            logger.warning(f"Decode aborted, retry with temp={temperature:.1f}")
        return res
    ...
    def asr(
        ...
        if self.verbose:
            rtf = t_total / total_duration if total_duration > 0 else 0
            logger.info(f"Qwen3-ASR stats: RTF={rtf:.3f}, duration={total_duration:.1f}s, "
                       f"encode={stats['encode_time']:.3f}s, "
                       f"prefill={stats['prefill_time']:.3f}s ({stats['prefill_tokens']}tok), "
                       f"decode={stats['decode_time']:.3f}s ({stats['decode_tokens']}tok)")
